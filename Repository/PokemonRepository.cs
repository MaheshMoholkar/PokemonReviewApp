...
namespace PokemonReviewApp.Repository
{
    public class PokemonRepository : IPokemonRepository
    {
        ...
        {
            var review = _context.Reviews.Where(p => p.Pokemon.PokemonId == pokemonId);
            if (review.Count() <= 0)
            {
                return 0;
            }
        ...
        {
            return _context.Pokemon.Any(p => p.PokemonId == pokemonId);
        }
        ...
        {
            var pokemonOwnerEntity = _context.Owners.Where(a => a.ownerId == ownerId).FirstOrDefault();
